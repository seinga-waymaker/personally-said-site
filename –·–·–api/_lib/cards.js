...
// Message deadline defaults to reveal date minus 3 days, and is never
// allowed later than reveal date minus 2 days (Phase 1 build spec, section
// 4). Dates are plain "YYYY-MM-DD" strings (no time zone math needed since
// both come from the same date picker). Returns '' if revealDate is missing
// or unparseable -- callers should fall back to whatever the client sent.
...
// Clamps a purchaser-supplied deadline to the reveal-date rule server-side
// (the date picker enforces this client-side too, but the server is the
// source of truth -- never trust the browser). Falls back to the default
// when the given deadline is missing, unparseable, or later than allowed.
