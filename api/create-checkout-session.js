...
//
// The order details go into the session metadata, which /api/stripe-webhook reads
// after payment to create the card code.

const Stripe = require('stripe');
const { TIERS, priceIdForTier } = require('./_lib/cards');
...
module.exports = async (req, res) => {
  ...
  // Use a saved Stripe Price if one is configured for this tier, otherwise an inline price.
  ...
  const metadata = {
    ...
    card_design: clean(body.design, 100),
    custom_track: clean(body.customTrackDetail, 500),
    price_id: priceId
  };
  ...
  try {
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [lineItem],
      metadata,
      // Same details on the payment itself, so they show on the Payments page.
